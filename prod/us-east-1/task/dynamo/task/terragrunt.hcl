...
locals {
  task        = read_terragrunt_config("${get_parent_terragrunt_dir()}/common/dynamodb/task/task.hcl")
  ...
  name        = "${local.common.locals.project_name}-${local.context.locals.name}-task-${local.environment.locals.name}"
}
