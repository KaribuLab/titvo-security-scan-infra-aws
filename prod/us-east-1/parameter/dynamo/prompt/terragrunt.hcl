...
locals {
  prompt      = read_terragrunt_config("${get_parent_terragrunt_dir()}/common/dynamodb/parameter/prompt.hcl")
  ...
  name        = "${local.common.locals.project_name}-${local.context.locals.name}-prompt-${local.environment.locals.name}"
}
